...
const INVENTORY_AUTH_USER_KEY = "inventory_auth_user";
const ALLOWED_ROLES = new Set(["inventory", "supervisor", "admin"]);
const isBrowser = typeof window !== "undefined";

function clearStoredUser() {
    if (!isBrowser) return;
    window.localStorage.removeItem(INVENTORY_AUTH_USER_KEY);
}

function saveStoredUser(user: User) {
    if (!isBrowser) return;
    window.localStorage.setItem(INVENTORY_AUTH_USER_KEY, JSON.stringify(user));
}
...
function hasInventoryAccess(roles: string[]): boolean {
    return roles.some((role) => ALLOWED_ROLES.has(role));
}

export const useInventoryAuthStore = create<InventoryAuthState>((set) => ({
    user: null,
    locked: false,
    hydrated: false,
    ...
    hydrate: () => {
        ...
        try {
            const rawUser = window.localStorage.getItem(INVENTORY_AUTH_USER_KEY);
            ...
            console.error("Inventory store hydrate error:", err);
            clearStoredUser();
    ...
    loginUser: async (passcode: string) => {
        ...
        try {
            ...
            if (!hasInventoryAccess(user.roles)) {
                return fail();
            }
